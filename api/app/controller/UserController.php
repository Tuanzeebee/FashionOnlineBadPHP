...
class UserController
{
    ...
    {
        // Kiểm tra xem tài khoản hiện tại là stylist hay không
        $current_user = $_SESSION['unique_id'];
        $check_sql = "SELECT is_stylist FROM users WHERE unique_id = {$current_user}";
        $check_query = mysqli_query($this->conn->connect(), $check_sql);
        $current_user_data = mysqli_fetch_assoc($check_query);

        if ($current_user_data['is_stylist'] == 1) {
            // Nếu tài khoản hiện tại là stylist, lấy danh sách người dùng không phải stylist
            $sql = "SELECT * FROM users 
                WHERE NOT unique_id = {$current_user}
                AND is_stylist = 0 
                ORDER BY user_id DESC";
        } else {
            // Nếu không phải stylist, lấy danh sách stylist
            $sql = "SELECT * FROM users 
                WHERE NOT unique_id = {$current_user}
                AND is_stylist = 1 
                ORDER BY user_id DESC";
